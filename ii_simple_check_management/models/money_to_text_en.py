...
def amount_to_text(number, currency):
    ...
    if start_word and end_word != "Zero":
        final_result = start_word+' '+_get_currency_name_by_code(currency)[0]+' '+' '+' '+end_word+" "+cents_name+" Only"
    if start_word and end_word == "Zero":
        final_result = ''
    if start_word != "Zero" and end_word == "Zero":
        final_result = start_word + ' ' + _get_currency_name_by_code(currency)[0] + " Only"
    if start_word == "Zero" and end_word != "Zero":
        final_result = end_word + " " + cents_name + " Only"
    return final_result
