...
class HrPayslip(models.Model):
    ...
    def _compute_input_line_ids(self):
        ...
        for slip in self:
            ...
            lines_to_remove = slip.input_line_ids.filtered(lambda x: x.input_type_id.is_loan)
            ...
            # input_type = self.env['hr.payslip.input.type'].sudo().search([]).filtered(lambda x:x.is_loan)
            ...
            for loan in loan_id:
                for loan_line in loan.loan_lines.filtered(lambda x:x.date):
                    if slip.date_from <= loan_line.date <= slip.date_to:
                        input_line_vals['input_type_id'] = loan.loan_type_id.input_id.id
                        input_line_vals['name'] = loan.loan_type_id.input_id.name
