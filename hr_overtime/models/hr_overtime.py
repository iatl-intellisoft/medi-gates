...
class HrOvertime(models.Model):
	...
	journal_id = fields.Many2one('account.journal', domain=[('type', '=', 'purchase')],requried=True)
	...
	def onch_emp_comp(self):
		self.working_day_rate = self.company_id.working_day_rate
		self.weekend_rate = self.company_id.weekend_rate
		self.public_holiday_rate = self.company_id.public_holiday_rate
	...
	def _get_total_amount(self):
		...
		for rec in self:
			...
			if rec.employee_id:
				...
				# overtime_rule = contract.structure_type_id.overtime_rule_id or rec.company_id.overtime_rule_id
				# rec.total_amount = 0.0
				# if overtime_rule:
	...
	def _amount_all(self):
		...
		for rec in self:
			...
			if rec.employee_id:
				...
				# if contract.structure_type_id.overtime_rule_id or rec.company_id.overtime_rule_id:
				# 	overtime_rule = contract.structure_type_id.overtime_rule_id or rec.company_id.overtime_rule_id
				...
				# if contract.employee_grade == 'm2' or contract.employee_grade == 'p1':
				# 	day = (contract.usd_salary * rec.company_id.usd_rate)/30
				# 	hour_wage = day/8
	            # else:
				# 	day = (contract.wage)/30
	...
	def _amount_all_config(self):
		...
		for rec in self:
			...
			if rec.employee_id:
				...
				working_day_rate = rec.company_id.working_day_rate
				weekend_rate = rec.company_id.weekend_rate
				public_holiday_rate = rec.company_id.public_holiday_rate
	...
	def action_approve(self):
		...
		# 						'name': rec.sequence,
		# 						'partner_id': rec.employee_id.work_contact_id.id,
		# 						'account_id': rec.employee_id.work_contact_id.property_account_payable_id.id,
		# 						'price_unit': rec.total_amount,
...
class OvertimeLine(models.Model):
	...
	hours = fields.Float('Hours', required='0',compute="_compute_hours")
	...
	@api.depends('date_from', 'date_to', 'overtime_type')
	def _compute_hours(self):
		calendar_obj = self.overtime_id.employee_id.resource_calendar_id
		hours_per_day = 0.0
		for hours in calendar_obj:
			hours_per_day = hours.hours_per_day
		for h in self:
			if h.date_from and h.date_to:
				diff = fields.Datetime.from_string(h.date_to) - fields.Datetime.from_string(h.date_from)
				diff = h.date_to - h.date_from
				duration = float(diff.days) * 24 + (float(diff.seconds) / 3600)
				# print ("diff---",diff,diff.days,type(diff))
				if h.overtime_type == 'weekend' or h.overtime_type == 'public_holiday' or h.overtime_type =='working_day':
					h.hours = duration
				else:                    
					h.hours = duration - hours_per_day
					if h.hours < 0:
						h.hours = 0.0
			else:
				h.hours = 0.0
...
class OverTimeBatch(models.Model):
	...
	def action_fetch(self):
		...
		"""
		overtime_ids = self.env['hr.overtime'].search([('start_date', '>=', self.start_date), ('state', '=', 'approve'), \
