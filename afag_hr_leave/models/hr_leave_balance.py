...
class HrLeaveBalance(models.Model):
    ...
    def _set_leave_balance(self):
        for rec in self.env['hr.employee'].search([('real_employee', '=', 'True')]).filtered(lambda x:x.real_employee == True):
            annual_leave = self.env['hr.leave.type'].sudo().search([('is_annual', '=', True)], limit=1)

            previous_allocation = self.env['hr.leave.allocation'].search([('employee_id', '=', rec.id),
                                                                          ('holiday_status_id', '=',
                                                                           annual_leave.id),
                                                                          ('state', '=', 'validate')], limit=1)

            previous_balance = self.env['hr.leave.balance'].search([('employee_id', '=', rec.id),
                                                                    ('leave_type', '=', annual_leave.id)], limit=1)

            today = fields.Date.today()
            daily_balance = 2.5 / 30


            if not previous_allocation and daily_balance > 0 and rec.join_date:
                delta = today - rec.join_date
                number_of_days = daily_balance * (delta.days+1)
                new_allocation = self.env['hr.leave.allocation'].sudo().create({
                    'name': 'Allocation for %s' % rec.name,
            ...
            if previous_allocation and daily_balance > 0 and previous_allocation.last_update:
