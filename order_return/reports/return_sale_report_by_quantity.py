...
class SaleReturnReportQuanitiy(models.Model):
    ...
    def _from_sale(self):
        ...
        return f"""
            ...
            LEFT JOIN stock_move_line sml ON sml.picking_id = s.sale_picking_id AND sml.product_id = l.product_id AND sml.qty_done > 0
            LEFT JOIN stock_move sm ON sm.id = sml.move_id AND sm.state = 'done'
            LEFT JOIN stock_picking sp ON sp.id = sm.picking_id AND sp.stock_return_id = s.id
